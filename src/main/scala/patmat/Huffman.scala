...
object Huffman {
  ...
  /**
    ...
    *   }
    */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy( x => x).toList.map( x => (x._1,x._2.length) ).sorted
  ...
  /**
    ...
    */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sorted.map(x => Leaf(x._1, x._2))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    trees match {
      case h1::h2::tail => (makeCodeTree(h1,h2) :: tail).toList
    }
  }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    ...
    until(singleton,combine)(leaf) match {
